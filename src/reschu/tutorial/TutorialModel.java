...
public abstract class TutorialModel {
    ...
    synchronized protected int getState() {
	return cur_state;
    }

    synchronized protected void setDuration(int sec) {
	duration = sec;
    }

    synchronized protected int getDuration() {
	return duration;
    }
    ...
    /**
     * If remaining time is equal to zero, then pops up a dialog with current
     * state If not, decrease the time by one second.
     */
    protected void tick() {
	if (getDuration() == 0)
	    showDialog();
