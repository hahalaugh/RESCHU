...
public class SituationAwareness {
     ...
     *            A vehicle
     * @param t
     *            A list of hazard areas
     * @param d
     *            Radius of a hazard area
     * @return True if intersects
     */
    public static int checkIntersect(final Vehicle v,
	    final LinkedList<int[]> haz, final int d) {
	LinkedList<int[]> path = new LinkedList<int[]>();
	path.addFirst(new int[] { v.getX(), v.getY() }); // add the vehicle's
							 // position to the
							 // first to the path
	...
	for (int i = 0; i < path.size() - 1; i++)
		...
		if (getDistance(path.get(i), path.get(i + 1), haz.get(j)) <= 15.0d) // pixel
										    // d
										    // doesn't
										    // work.
										    // so
     ...
     * @return
     */
    private static double getDistance(final int[] A, final int[] B,
	    final int[] C) {
	...
	// System.out.println("DISTANCE AB-C ("+ax+","+ay+")("+bx+","+by+")-("+cx+","+cy+") = "
	// + distanceSegment);
