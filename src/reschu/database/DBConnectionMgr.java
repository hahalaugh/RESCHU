...
public class DBConnectionMgr {
	...
	for (int i = 0; i < count; i++) {
	    ...
	    trace("ConnectionPoolManager: Adding new DB connection to pool ("
		    + connections.size() + ")");
	    NumConnections++;
	}
    }
    ...
    /**
     * Returns an unused existing or new connection. if the number of current
     * connections exceeds maxConnections, throws an exception.
     */
    public synchronized Connection getConnection() throws Exception {
	if (!initialized) {
	    DriverManager.registerDriver((Driver) Class.forName(_driver)
		    .newInstance());
	    initialized = true;
	}
	...
	for (int i = 0; i < connections.size(); i++) {
		...
		if (badConnection) {
		    ...
		    trace("ConnectionPoolManager: Remove disconnected DB connection #"
			    + i);
		    continue;
		}
		...
		trace("ConnectionPoolManager: Using existing DB connection #"
			+ (i + 1));
		break;
	    }
	}
	...
	if (c == null) {
		...
		throw new Exception(
			"ConnectionPoolManager: Unable to create more connections. Limit exceeded. (#"
				+ connections.size() + ")");
	    }
	    c = createConnection();
	    ...
	    trace("ConnectionPoolManager: Creating new DB connection #"
		    + connections.size());
	}
