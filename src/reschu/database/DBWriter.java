...
public class DBWriter {
    ...
    /**
     * In order to be able to check the actual order of events, this variable is
     * incremented every time when there is a write request.
     */
	...
	if (!Reschu.train() && !Reschu.tutorial()) { // writes to database only
						     // if the game is in
						     // experiment mode
		...
		public void run() {
			...
			if (PRINT_QUREY_RESULT)
			    System.err.println("DB: " + sql + " finished");
		    } catch (Exception e) {
			System.err.println("Error: failed to execute sql. "
				+ sql);
		    }
	...
	// table name doesn't support the email format. So we regularize the
	// username.
	...
	final String sql = "CREATE TABLE " + table_username + " ("
		...
		+ "type TINYINT NOT NULL DEFAULT 0, " + "vehicle TINYINT, "
		+ "log varchar(128) NOT NULL DEFAULT 'none', "
		+ "mouseCoordX SMALLINT, " + "mouseCoordY SMALLINT" + ")";
	ExecuteSQL(sql);
    }

    public void Write(String tableName, int msecTime, int invoker, int type,
	    int vIdx, String log, int mouseCoordX, int mouseCoordY) {
	...
	final String sql = "INSERT INTO "
		+ table
		+ "(sortIdx, gameTime, invoker, type, vehicle, log, mouseCoordX, mouseCoordY) VALUES ("
		+ getOrder() + ", " + "'" + setTimeFormat(msecTime) + "', "
		+ invoker + ", " + type + ", " + vIdx + ", " + "'" + log
		+ "', " + mouseCoordX + ", " + mouseCoordY + ")";
	ExecuteSQL(sql);
    }

    /*
     * public void UserTable_SetState(String username, int state) { final String
     * sql = "UPDATE USER SET state=" +state + " WHERE username='" + username +
     * "'"; ExecuteSQL(sql); }
     */
    public void UserTable_SetGameStart(String username) {
	final String sql = "UPDATE USER SET GameStart=1 WHERE username='"
		+ username + "'";
	ExecuteSQL(sql);
    }

    public void UserTable_SetGameFinish(String username) {
	final String sql = "UPDATE USER SET GameFinish=1 WHERE username='"
		+ username + "'";
	ExecuteSQL(sql);
    }

    public void UserTable_SetTutorialFinish(String username) {
	final String sql = "UPDATE USER SET TutorialFinish=1 WHERE username='"
		+ username + "'";
	...
	final String sql = "UPDATE USER SET last_login=CURRENT_TIMESTAMP WHERE username ='"
		+ username + "'";
	ExecuteSQL(sql);
    }

    public void ScoreTable_SetScore(String username, double score) {
    ...
    /**
     * Change an irregular username format to durable format so that we can
     ...
     */
    private synchronized void incrOrder() {
	sortIdx++;
    }
